...
dir_open(const char *dn)
{
	...
	fprintf(stdout, "\t%ld File(s) %ld bytes\n", file_count, total_file_size);

	/* Print the directory count */
	fprintf(stdout, "\t%ld Dir(s)\t\n", dir_count);
	return 0;
}

/* The entry point to the command */
int
dir_entry(const char **argv)
{
	if (!argv)
		return dir_open(".");
